...
class NavBlockEnhancements
{
    ...
    public function processNavigationBlock( string $block_content, array $block ): string
    {
        ...
        $on_click = match( true ) {
            isset( $block['attrs']['submenuVisibility'] ) => $block['attrs']['submenuVisibility'] === 'click',
        ...
        if ( 'vertical' === $orientation ) {
            ...
            while( $processor->next_tag() ) {
                ...
                */
                ...
                if ( ! $on_click ) {
                    /**
                     * If not opening on click, expand all menus all the time
                     * Hover makes no sense in a vertical layout
                     */
                    // if ( 'false' === $processor->get_attribute( 'aria-expanded' ) ) {
                    //     $processor->set_attribute( 'aria-expanded', 'true' );
                    // }
                    // /**
                    //  * If menus are always expanded, no interactivity is required
                    //  */
                    // $processor->remove_attribute('data-wp-context');
                    // $processor->remove_attribute('data-wp-interactive');
                    // $processor->remove_attribute('data-wp-on--focusout');
                    // $processor->remove_attribute('data-wp-on--keydown');
                    // $processor->remove_attribute('data-wp-on--mouseenter');
                    // $processor->remove_attribute('data-wp-on--mouseleave');
                    // $processor->remove_attribute('data-wp-watch');
                    // $processor->remove_attribute('data-wp-on--focus');
                    // $processor->remove_attribute('data-wp-on--click');
                    // $processor->remove_attribute('data-wp-bind--aria-expanded');
                }
            }
